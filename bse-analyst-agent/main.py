...
def run_universe_scan(refresh: bool = False, top: int = 50, limit: int | None = None, output_dir: str = OUTPUT_DIR) -> List[Dict[str, Any]]:
    ...
    candidates.sort(key=lambda row: (0 if row["market_cap_category"] == "MICROCAP" else 1, -float(row["market_cap_cr"]), -float(row["avg_daily_value_cr"])))
    ...
    for index, row in enumerate(selected, 1):
        print(f"{index:>2}. {row['symbol']:<15} {row['market_cap_category']:<9} MCap ₹{float(row['market_cap_cr']):>9.0f} Cr  Price ₹{float(row['price']):>8.2f}  Traded ₹{float(row['avg_daily_value_cr']):>7.2f} Cr")
    return selected
...
def _show_result(result: Dict[str, Any]) -> None:
    ...
    print("=" * 72)
    for key in ("status", "stage", "verdict", "decision_score", "quality_score", "governance_grade", "corporate_risk_score", "annual_reports_scanned", "fair_value", "buy_below"):
        if key in result:
...
def run_single_stock(symbol: str, live_filings: bool = True) -> Dict[str, Any]:
    """Analyze a single stock and generate all reports automatically."""
    result = DeepScannerEngine().analyze_symbol(symbol, live_filings=live_filings)
    _show_result(result)
    if result.get("status") == "ANALYZED":
        _generate_all_reports(os.path.join(OUTPUT_DIR, symbol.upper().strip()))
    return result
...
def interactive() -> None:
    while True:
        ...
        if choice == "1":
            ...
            if answer in ("", "y", "yes"):
                DeepScannerEngine().run(input_csv=UNIVERSE_FILE, top=10, deep_limit=20, live_filings=True)
        elif choice == "3":
            print("Exiting.")
            return
        else:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Indian equity research agent and small/micro-cap scanner")
...
def main() -> None:
    ...
    if args.scan:
        ...
        return
    if args.deep_scan:
        DeepScannerEngine().run(input_csv=args.input_csv, top=args.top, deep_limit=args.deep_limit, live_filings=live_filings)
        return
    if args.symbol:
