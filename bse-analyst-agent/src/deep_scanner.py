...
class DeepScannerEngine:
    ...
    def analyze_candidate(
        ...
        try:
            filing_data, corporate, reports, audits = self._load_governance_evidence(
            ...
            decision = calculate_investment_decision(
                ...
                valuation=valuation,
                governance_grade=corporate.governance_grade,
                forensic_clean=not corporate.hard_fail,
            )
            ...
            result = {
                **row,
                ...
                "ai_conviction": getattr(memo, "conviction_score", None),
                ...
                "filing_source": filing_data.get("source", "NSE"),
                "annual_reports_scanned": len(audits),
                "annual_report_paths": ";".join(str(record["path"]) for record in reports),
                ...
                "risk_flags": ";".join(getattr(memo, "critical_risks", []) or []),
                "thesis": str(getattr(memo, "executive_summary", "") or "").strip(),
            }
    ...
    def _persist_stage_outputs(
        ...
        if history is not None and ratios is not None and quality is not None:
            self._write_json(
                os.path.join(output_dir, "financial_analysis.json"),
                {
                    "symbol": symbol,
                    "analysis_type": "financial_analysis",
                    "financial_score": quality["score_100"],
                    "financial_components": quality["components"],
                    "ratios": dict(ratios),
                    "valuation": valuation or {"available": False},
                    "history": [year.model_dump() for year in history.years],
                },
            )

        if decision is not None:
            self._write_json(
                os.path.join(output_dir, "investment_decision.json"),
                {
                    "symbol": symbol,
                    "analysis_type": "investment_decision",
                    **decision,
                    "governance": {
                        "grade": corporate.governance_grade,
                        "risk_score": corporate.risk_score,
                        "annual_report_score": corporate.annual_report_score,
                        "hard_fail": corporate.hard_fail,
                    },
                    ...
                    "ai": {
                        "conviction_score": getattr(memo, "conviction_score", None),
                        "thesis": str(getattr(memo, "executive_summary", "") or "").strip(),
                    },
                },
            )
